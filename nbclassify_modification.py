...
sortHamDict = sorted(hamDictionary.items(),key=operator.itemgetter(1),reverse=True)
topHamWords = [k[0] for k in sortHamDict[0:100]]
sortSpamDict = sorted(spamDictionary.items(),key=operator.itemgetter(1),reverse=True)
topSpamWords = [k[0] for k in sortSpamDict[0:100]]
...
for cw in commonWords:
    del hamDictionary[cw]
    del spamDictionary[cw]
#delete words that have length less than or equal {threshold_key_length}
minKeyLength = int(3)
keys = [key for key in hamDictionary]
keys = [key for key in keys if len(key) <= minKeyLength]
for k in keys:
    del hamDictionary[k]
keys = [key for key in spamDictionary]
keys = [key for key in keys if len(key) <= minKeyLength]
for k in keys:
    del spamDictionary[k]
